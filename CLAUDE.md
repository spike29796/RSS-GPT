...
**工单住共享区 `tickets/`（T-XXX.md）、需求住共享区 `requirements/`（REQ-*.md）**——两样不进 git、不 push；
仓库 `docs/` 只留模块文档 MOD-*.md（跟 git 走）。
...
- **commit 消息规范（2026-08-13 包工头定）**：一律 `T-XXX YYYY-MM-DD`（如 `T-032 2026-08-13`），
  同日同单多个节点时补时分（`T-032 2026-08-13 14:30`）。禁止长串描述。
- 包工头本轮吩咐的任务全部 PASS 后，头头 push 一次。
- **push 内容红线（2026-08-13 包工头定）**：工单 tickets/、需求 requirements/、内部文档、测试脚本一律不 push；
  GitHub 上只出现业务代码、手册、MOD 文档。
